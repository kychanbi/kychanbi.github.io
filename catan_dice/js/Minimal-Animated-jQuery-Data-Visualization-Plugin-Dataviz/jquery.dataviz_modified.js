...
(function($) {
    ...
    $.DataViz = function(element, options) {
        ...
        plugin.draw_bar = function(){
            ...
            if (width<480){
                var x_start = width/12; var  x_width= x_start-5;
            }
            else{
                var x_start= 60, x_width= 50;
            }
            for (i; i<o.loop; i++) {
				var x = (x_start*i)+x_start/2;
				// create bars and tooltips
				$element.append('<canvas id="bar_'+i+'" class="bars" style="left:'+x+'px" width="'+x_width+'" height="300"></canvas>');
				$element.append('<canvas id="bar_tooltip_'+i+'" class="bar_tooltips" style="left:'+x+'px" width="'+x_width+'" height="300"></canvas>');
